...
static sockaddr_in l_addr;
static int l_addr_len = sizeof(sockaddr_in);
static sockaddr_in r_addr;
static int r_addr_len = sizeof(sockaddr_in);
//新接收的客户端ip
static char ip[16] = { 0 };
//新接收的客户端端口
int port = 0;
...
bool CMasterServerImplIOCP::IocpAccept(int cSock)
{
	...
	iocp_.GetAddr(&io_data_, &l_addr, l_addr_len, &r_addr, r_addr_len);
	strncpy(ip, inet_ntoa(r_addr.sin_addr), 16);
	port = ntohs(r_addr.sin_port);
	...
	if (client_num_ < max_client_num_)
	{
		...
		//设置client为websocket
		pclient->SetSocketType((enum SocketType)socket_type_);
		pclient->SetProtocalType((enum ProtocalType)protocal_type_);
		//触发客户端加入事件
		OnJoin(pclient);
	}
	else
...
//创建多个消息处理客户端并启动
void CMasterServerImplIOCP::Start()
{
	//开启所有消息处理服务端线程
	for (int i = 0; i < cmsg_server_num_; ++i)
	{
		CAbstractWorkerServer *pcmsg_server = new CWorkerServerImplIocp();
		pcmsg_server->Init();
		pcmsg_server->set_accept_server(this);
		pcmsg_server->Start();
