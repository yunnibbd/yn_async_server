...
char *Crypt::Base64Decode(const uint8_t* text, size_t sz, int* out_size)
{
...
char *Crypt::Base64Encode(const uint8_t* text, int sz, int* out_esz)
{
...
void Crypt::Base64EncodeFree(char* result)
{
	delete[] result;
}
...
void Crypt::Sha1(uint8_t* buffer, int sz, uint8_t* digest, int* e_sz) 
{
	SHA1_CTX ctx;
	sat_SHA1_Init(&ctx);
...
void Crypt::Md5(const char *message, long len, char *output) 
{
	WORD32 d[4];
	int status = 0;
	long i = 0;
