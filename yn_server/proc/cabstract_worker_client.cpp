...
void CAbstractWorkerClient::Init()
{
	...
	if (!is_connected_)
	{
		LOG_INFO("connct to server %s:%d failed\n", server_ip_, server_port_);
		return;
	}
	LOG_INFO("connct to server %s:%d success\n", server_ip_, server_port_);
	//触发连接成功事件
	OnConnected();
}

//收到消息时的处理, 可覆盖
void CAbstractWorkerClient::OnMsg(CBaseMsg *msg)
{
	LOG_DEBUG("受到了数据\n");
}
...
bool CAbstractWorkerClient::StartConnect()
{
	...
	if (sockfd_ == SOCK_NOT_INIT) InitSocket();
	...
	if (::connect(sockfd_, (sockaddr*)&sin, sizeof(sin)) == SOCK_ERROR)
	{
