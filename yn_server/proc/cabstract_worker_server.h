...
public:
	...
	//处理消息回调
	virtual void OnMsg(CClient *pclient, CBaseMsg *msg) override;

	//处理二进制回调
	virtual void OnBin(CClient *pclient, char *data, int data_len) override;
	...
	//使用消息传输方式进行解析
	bool OnMsgProtocalRecv(CClient *pclient);

	//使用二进制方式进行解析
	bool OnBinProtocalRecv(CClient *pclient);
	...
	void set_accept_server(CAbstractMasterServer *p) { accept_server_ = p; }

protected:
	//本消息处理服务端的Accept服务端(用于触发它的事件)
	CAbstractMasterServer *accept_server_ = nullptr;
...
private:
	...
	CThread thread_;
	//消息线程的线程池对象指针
	CThreadPool *msg_thread_pool_ = nullptr;
};
