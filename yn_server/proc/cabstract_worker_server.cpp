...
void CAbstractWorkerServer::Init()
{
	...
	//开启多少个线程来处理消息
	int msg_thread_nums = p_config->GetIntDefault("MsgProcessThreadNums", 2);
	//是否开启心跳检测
	is_check_heart_ = p_config->GetIntDefault("IsCheckHeart", 1);
	msg_thread_pool_ = CThreadPoolFactory::Create();
	msg_thread_pool_->Init(msg_thread_nums);
}
...
{
	if (accept_server_)
		accept_server_->OnLeave(pclient);
	//最后再删除
	delete pclient;
}
...
//使用消息传输方式进行解析
bool CAbstractWorkerServer::OnMsgProtocalRecv(CClient *pclient)
{
	auto p_recv_buf = pclient->p_recv_buf();
	while (p_recv_buf->DataLen() >= sizeof(CBaseMsg))
	{
		auto msg = (CBaseMsg *)p_recv_buf->Data();
		//触发处理msg回调
		OnMsg(pclient, msg);
		//移除已处理的数据
		p_recv_buf->Pop(msg->data_length_);
	}
	return true;
}

//使用二进制方式进行解析
bool CAbstractWorkerServer::OnBinProtocalRecv(CClient *pclient)
{
	int pack_len = 0;
	auto p_recv_buf = pclient->p_recv_buf();
	while (p_recv_buf->DataLen() > 0)
	{
		...
		//前两个字节作为本次接收的长度
		pack_len = (data[0]) | (data[1] << 8);
		if (pack_len > MAX_PKG_SIZE)
			//异常数据包
			return false;
		if (p_recv_buf->DataLen() >= pack_len)
			//有一个完整的包, 触发接收二进制处理回调
			OnBin(pclient, data + 1, pack_len - 2);
		else
			//没有收到完整的包体
			return true;
		//移除处理完的数据
		p_recv_buf->Pop(pack_len + 2);
	}
	return true;
}
...
bool CAbstractWorkerServer::StartWSConnect(CClient *pclient)
{
	...
	Crypt::Sha1((uint8_t *)key_migic, (int)strlen(key_migic), sha1_content, &sha1_size);
	char* b64_str = Crypt::Base64Encode(sha1_content, sha1_size, &base64_len);
	//end 
	...
	Crypt::Base64EncodeFree(b64_str);
	//将所有的前置信息清空
	p_recv_buf->Pop(p_recv_buf->DataLen());
	return true;
}

static bool recv_ws_header(char *pkg_data, int pkg_len,
...
void CAbstractWorkerServer::ParserWsPack(CClient *pclient, char* body, int body_len, char* mask, int protocal_type)
{
	...
	if (protocal_type == JSON_PROTOCAL)
		...
		OnBin(pclient, body, body_len);
	else if (protocal_type == MSG_PROTOCAL)
		OnMsg(pclient, (CBaseMsg *)body);
}
...
void CAbstractWorkerServer::ProcessMsg()
{
	...
	for (; iter != end;)
	{
		...
		{
		case TCP_SOCKET_IO:
			if (protocal_type == MSG_PROTOCAL)
			{
				if (!OnMsgProtocalRecv(pclient))
					goto close_client;
			}
			else if (protocal_type == BIN_PROTOCAL)
			{
				if (!OnBinProtocalRecv(pclient))
					goto close_client;
			}
...
//此函数由accept服务端调用, 将新接收的客户端加入本消息服务端的客户端缓冲队列
...
//收到到客户端消息事件
void CAbstractWorkerServer::OnMsg(CClient *pclient, CBaseMsg *msg)
{
	LOG_DEBUG("收到消息 client %d\n", pclient->test_id);
	LOG_DEBUG("%s\n", msg->username);
}

//处理二进制回调
void CAbstractWorkerServer::OnBin(CClient *pclient, char *data, int data_len)
{
	LOG_DEBUG("收到二进制 client %d\n", pclient->test_id);
...
{
	LOG_DEBUG("收到json client %d\n", pclient->test_id);
	data[data_len] = 0;
	LOG_DEBUG("%s\n", data);
	pclient->WSSendData(data, data_len);
}
