...
void CAbstractMasterServer::Init()
{
	...
	//开启多少个消息处理服务端线程
	cmsg_server_num_ = p_config->GetIntDefault("MsgServerNum", 2);
	//心跳超时配置读取
	heart_check_timeout_ = p_config->GetIntDefault("HeartCheckTimeout", 10000);
	//创建的客户端的socket类型
	socket_type_ = p_config->GetIntDefault("SocketType", 0);
	//创建的客户端的传输协议类型
	protocal_type_ = p_config->GetIntDefault("ProtocalType", 0);
	//初始化监听socket
	InitSocket();
	//让本类Main函数作为一个线程启动
	thread_.Start(
...
{
	if (server_port_ == SERVER_PORT_NOT_INIT)
	{
	...
	if (::bind(sockfd_, (sockaddr*)&sin, sizeof(sin)) == SOCK_ERROR)
	{
		LOG_ERROR("socket bind failed\n");
		return;
	}
	if (::listen(sockfd_, 512) == SOCK_ERROR)
	{
